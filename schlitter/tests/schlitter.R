...
## covariance matrix
dat.cov = cov(dat);
write.table(dat.cov, file = "debug_covar_R.dat", quote = FALSE, col.names = FALSE);
...
eval_lim = min(length(dat.eigen$values), dim(dat)[1]) - 6;
S_sch = vector("numeric", length = eval_lim);
S_sch = sapply(dat.eigen$values[1:eval_lim], function(x) {
		return(Nav * 0.5 * k_B * log(1 + (prefr *  cf_au_kg * m_CH * x * cf_nmsq_msq)));
	}
)

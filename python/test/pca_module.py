...
##
freedcdfile = DCDFile(args.freedcdfile)
restdcdfile = DCDFile(args.restdcdfile)
freepdbfile = parsePDB(args.freepdbfile)
#print restpdbfile
...
def trajpca_superimposed(freepdbfile, freedcdfile, restpdbfile, restdcdfile):
    freestruct = parsePDB(freepdbfile)
    freedcd = Trajectory(freedcdfile)
    nframes = freedcd.numCoordsets()
    exit()
    freedcd.addFile('bound.dcd')
...
trajpca_superimposed(freepdbfile, freedcdfile, restpdbfile, restdcdfile)
